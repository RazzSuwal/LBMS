...
import { Billing, BillingVM, Book, BookCategory, Booked, Books, BooksWithPagination, Customer, Order, Product, User, UserType } from '../../models/models';
...
export class ApiService {
  ...
  getBookById(id: number): Observable<Book> {
    const url = `${this.baseUrl}GetBooksById?id=${id}`;
    return this.http.get<Book>(url);
  }
  editBook(id: number, book: Booked): Observable<Booked> {
    const url = `${this.baseUrl}EditBook?id=${id}`;
    return this.http.put<Booked>(url, book);
  }

  getBooksWithPagination(searchTerm: string = '', pageNumber: number = 1, pageSize: number = 10): Observable<BooksWithPagination[]> {
    let params = new HttpParams()
        .set('searchTerm', searchTerm || '')
        .set('pageNumber', pageNumber.toString())
        .set('pageSize', pageSize.toString());

    return this.http.get<BooksWithPagination[]>(`${this.baseUrl}GetBooksWithPagination`, { params });
}
  ...
  createBilling(billngvm: BillingVM) {
    return this.http.post(this.baseUrl + 'CreateBilling', billngvm, {
      responseType: 'text',
    });
  }
  ...
  addNewBilling(billing: Billing) {
    return this.http.post(this.baseUrl + 'AddBilling', billing, {
      responseType: 'text',
    });
  }
